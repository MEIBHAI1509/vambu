...
export default function GroupsPage() {
  ...
  const { openCreateGroupModal } = useChatWorkspace()
  ...
  if (loading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center rounded-3xl bg-card border border-border min-h-[200px]">
        ...
        <p className="text-muted-foreground animate-pulse text-sm">Loading groups…</p>
      </div>
    )
  }
    ...
    <div className="flex-1 flex flex-col items-center justify-center min-w-0 min-h-0 bg-[var(--chat-surface)] p-8 text-center">
      ...
      >
        ...
        <p className="text-sm text-muted-foreground">
          Create a group or open one from the list. For direct messages, use{' '}
          <Link href="/chat" className="font-medium text-[#141235] dark:text-indigo-400 underline underline-offset-2">
            Messages
          </Link>
          .
        </p>
